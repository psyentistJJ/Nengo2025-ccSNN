...
from algorithm.model.weight_init import almost_xavier_uniform_,almost_xavier_normal_
...
# network parameters
batch_size =256# 32  # unknown
optim_class = torch.optim.Adam
optim_params = (0.9,0.999) #betas
scheduler=None
learning_rate = 0.001#0.001
weight_init_ = almost_xavier_normal_
target_weights =None #None or 'target_model'
Dales_law=0.7 #None of fraction of exc neurons
N_train_only_weights=1
...
train_weights=False #train weights
train_out=True #train model on loss after output layer (=conventional training on loss)
train_hidden=False #train model on loss from hidden layer (e.g. comparing to teacher's hidden layer)
weight_scaling_factor=1.0
recurrent_sparse=0.5
low_rank_sparse=None
train_intrinsic=['I_c', 'beta', 'w1','wsf'] #train neuron-intrinsic parameters, train w1 only during last N_train_only_weights epochs
synapse = recurrent_synapse #choose forward or recurrent synapse
training_mode = 'classification_BLK' #classification: learn based on 'true' classes, vs trace-learning: based on teacher network
hidden_mode=None
target_model = None #teacher network
params_from_target =None #which neuron-intrinsic parameters are copied from teacher network
hidden_in_loss = None # indices/percentage of hidden layer neuyrons considered for the loss of the hidden layer
hidden_loss_scale=None #scaling factor for hidden loss
noise=None #if parameter is initialized from target model (in params_from_target) and trained as well, add this factor *gaussian noise to the parameter init
